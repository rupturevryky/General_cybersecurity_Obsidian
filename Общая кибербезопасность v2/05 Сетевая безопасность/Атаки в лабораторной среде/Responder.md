...
verified_against: https://github.com/lgandx/Responder
---

> [!warning] Сначала анализ без ответов
> Начинайте с Analysis mode `-A`, который не отравляет ответы. Режимы ответов и relay разрешены только в назначенной лаборатории внешней учебной платформы.
...
> [!warning] Пароль в аргументе процесса
> Значение `-p` может попасть в историю shell и список процессов. Используйте только одноразовые лабораторные данные и очищайте историю учебной VM после занятия.
## Атака HTTP Auth
**Команда для запуска режима:**
```python
responder -b -I eth0
