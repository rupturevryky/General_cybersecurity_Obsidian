...
> В John файл хешей является позиционным аргументом; результаты читаются через `john --show <HASH_FILE>`. Используйте только тестовые хеши, выданные внутри назначенного задания внешней платформы.
