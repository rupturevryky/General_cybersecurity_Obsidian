...
![[Учебный курс — кибербезопасность/01 Основы/Сети/Передача файлов/assets/Pasted image 20240716082210.png]]
```
[global]
workgroup = WORKGROUP
security = user
...
![[Учебный курс — кибербезопасность/01 Основы/Сети/Передача файлов/assets/Pasted image 20240716221048.png]]
...
```
Теперь перезапустим **samba** командой `systemctl restart smbd` и убедимся в том, что все работает, зайдя в нашу папку со сторонней **windows** машины. Для этого в проводнике вместо пути пропишите **ip**-адрес файлового сервера (узнать его можно с помощью команды ip a):![[Учебный курс — кибербезопасность/01 Основы/Сети/Передача файлов/assets/Pasted image 20240716082454.png]]
...
![[Учебный курс — кибербезопасность/01 Основы/Сети/Передача файлов/assets/Pasted image 20240716222611.png]]
...
Samba file server и Samba AD Domain Controller — разные роли, которые проектируют и защищают отдельно. Для общего ресурса определяют владельца данных, допустимые пользователи/группы, effective filesystem ACL и share permissions, необходимость guest access, шифрование/signing и разрешённые сети.
...
`testparm` проверяет и выводит конфигурацию, но не доказывает корректность бизнес-доступа. Итоговые права зависят одновременно от Samba и файловой системы. Не открывайте SMB в Интернет; legacy SMB1 отключают, если нет документированной изолированной зависимости. Firewall настраивают средством, управляющим конкретной системой, а не устанавливают `iptables-persistent` автоматически.

## На собеседовании

1. Чем Samba отличается от протокола SMB?
2. Как совместно применяются share permissions и filesystem ACL?
3. Почему успешный `testparm` не доказывает безопасность ресурса?
4. Зачем нужны SMB signing и encryption?

## Внешняя практика

В разрешённой лаборатории сопоставьте share с путём, пользователями, effective ACL, сетевой экспозицией и журналами. Не изменяйте доступ и не загружайте файлы вне задания.
