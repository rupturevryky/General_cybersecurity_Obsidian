...
> [!abstract] Цель
> Понять роль веб-сервера, виртуальных хостов и основных конфигурационных каталогов Apache и Nginx, затем исследовать их конфигурацию в назначенной лаборатории внешней учебной платформы.

> [!warning] Актуальность конфигурации
> Пути, репозитории, ключи подписи и параметры зависят от дистрибутива и версии. Установочные команды приведены для разбора; в рамках курса запускайте их только на машине, предоставленной внешней учебной платформой.
...
Зайдем на [localhost](http://localhost) чтобы убедиться что все работает:![[Учебный курс — кибербезопасность/01 Основы/Web и HTTP/assets/Pasted image 20240716083408.png]]
...
И вставляем туда базовые настройки:![[Учебный курс — кибербезопасность/01 Основы/Web и HTTP/assets/Pasted image 20240716083608.png]]
...
Осталось прописать доменное имя в файле `/etc/hosts`:![[Учебный курс — кибербезопасность/01 Основы/Web и HTTP/assets/Pasted image 20240716083640.png]]
И перейти по нему в браузере для проверки:![[Учебный курс — кибербезопасность/01 Основы/Web и HTTP/assets/Pasted image 20240716083652.png]]
...
```
![[Учебный курс — кибербезопасность/01 Основы/Web и HTTP/assets/Pasted image 20240716083930.png]]
...
Посмотрим на результат нашей работы, перейдя по адресу нашего хоста в браузере:![[Учебный курс — кибербезопасность/01 Основы/Web и HTTP/assets/Pasted image 20240716083952.png]]
...
![[Учебный курс — кибербезопасность/01 Основы/Web и HTTP/assets/Pasted image 20240717192948.png]]
...
```
![[Учебный курс — кибербезопасность/01 Основы/Web и HTTP/assets/Pasted image 20240717193054.png]]
В `/var/log/nginx/error.log` будут отображаться ошибки превышения количества запросов пользователями.
![[Учебный курс — кибербезопасность/01 Основы/Web и HTTP/assets/Pasted image 20240717193235.png]]
...
maxretry = 4
```
![[Учебный курс — кибербезопасность/01 Основы/Web и HTTP/assets/Pasted image 20240717193400.png]]
...
iptables -L -v
```
![[Учебный курс — кибербезопасность/01 Основы/Web и HTTP/assets/Pasted image 20240717193654.png]]
Убедимся. что настройки в **fail2ban** применились и введём: `fail2ban-client status`![[Учебный курс — кибербезопасность/01 Основы/Web и HTTP/assets/Pasted image 20240717193725.png]] _видим, что фильтры работают по nginx-limit-req, sshd_
```
fail2ban-client status nginx-limit-req
```
![[Учебный курс — кибербезопасность/01 Основы/Web и HTTP/assets/Pasted image 20240717193806.png]]

## Безопасный рабочий порядок

Перед изменением установите цепочку `клиент → proxy/CDN → веб-сервер → приложение`, владельца конфигурации и способ автоматического развёртывания. Ручная правка файла, которым управляет configuration management, будет перезаписана.
...
Сначала проверяют синтаксис и итоговые virtual hosts, затем применяют reload и проверяют функцию и журнал. Имена команд и unit зависят от системы. Не публикуйте document root, backup-файлы, status endpoints и административные интерфейсы; ограничивайте методы, размер запросов и права service account. Версию и заголовки нельзя считать достаточной мерой защиты — важны обновления и конфигурация.

Fail2ban и rate limiting уменьшают часть автоматизированных попыток, но не заменяют аутентификацию, исправление уязвимости и upstream-защиту. Правила firewall могут управляться nftables, firewalld, ufw или платформой; не предполагайте iptables backend автоматически.

## На собеседовании

1. Чем virtual host отличается от отдельного IP-сервера?
2. Зачем проверять конфигурацию до reload?
3. Чем reload отличается от restart?
4. Где искать реальный адрес клиента за trusted reverse proxy?
5. Почему fail2ban не исправляет слабую аутентификацию?

## Внешняя практика

В разрешённой лаборатории исследуйте итоговую конфигурацию одного virtual host, document root, proxying, TLS, пользователя процесса и журналы. Изменения выполняйте только по заданию платформы.
