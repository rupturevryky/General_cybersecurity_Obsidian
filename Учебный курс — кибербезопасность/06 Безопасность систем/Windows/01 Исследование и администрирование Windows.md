...
verified_against: https://learn.microsoft.com/windows-server/administration/windows-commands/windows-commands
---

> [!note] Проверка источников завершена
> Базовая линия: Microsoft Learn. Исторические примеры сохранены для разбора, но не являются рекомендацией для реальных систем; практику выполняйте только в назначенной лаборатории внешней учебной платформы и в пределах её правил.
...
![[Учебный курс — кибербезопасность/01 Основы/Сети/05 Протоколы удалённого доступа#SMBMAP]]

![[Учебный курс — кибербезопасность/01 Основы/Сети/05 Протоколы удалённого доступа#SMBCLIENT]]
...
![[Учебный курс — кибербезопасность/06 Безопасность систем/Windows/assets/Pasted image 20250917180201.png]]
...
Другие полезные классы WMI/CIM, которые в современном PowerShell следует запрашивать через `Get-CimInstance`:
...
`Get-WmiObject` относится к Windows PowerShell 5.1 и оставлен ниже для распознавания старых сценариев. В новых сценариях используйте CIM-команды (`Get-CimInstance`, `Invoke-CimMethod`) и PowerShell Remoting с минимально необходимыми правами.
...
Исторически эти задачи выполнялись через WMIC. WMIC устарел и в новых Windows может быть отключён или отсутствовать; примеры ниже нужны для чтения legacy-сценариев. Для нового кода используйте PowerShell CIM cmdlets.
...
```powershell-session
PS C:\lab> Get-CimInstance -ClassName Win32_OperatingSystem | Select-Object SystemDirectory,BuildNumber,SerialNumber,Version | Format-Table
