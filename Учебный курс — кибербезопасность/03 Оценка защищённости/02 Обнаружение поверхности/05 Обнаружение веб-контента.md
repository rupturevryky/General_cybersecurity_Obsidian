...
verified_against: https://github.com/OJ/gobuster
---

> [!note] Проверено для Gobuster 3.8.2
> Текущая сборка требует Go 1.24+. Режимы имеют раздельный синтаксис; скорость запросов ограничивайте согласно scope и устойчивости стенда.
...
> [!warning] Активная проверка
> Перебор путей способен создать значительную нагрузку и заполнить журналы. Согласуйте словарь, расширения, число потоков, частоту запросов и исключения.

Инструменты ниже ищут доступные маршруты и файлы по словарю. Наличие пути не является уязвимостью само по себе: оцените назначение ресурса, требования аутентификации и раскрываемые данные.
...
или 
```
gobuster dns -w /usr/share/wordlists/dirb/subdomains-top1million-5000.txt -do <DOMAIN>
```
*man gobuster **EXAMPLES**:*
```bash
gobuster dir -u https://example.test/path/to/folder -c 'session=example' -t 10 -w common-files.txt -x php,html

gobuster dns -do example.test -t 50 -w common-names.txt

       gobuster s3 ‐w bucket‐names.txt

       gobuster gcs ‐w bucket‐names.txt

gobuster vhost -u https://example.test -w common-vhosts.txt

gobuster fuzz -u 'https://example.test/?FUZZ=test' -w parameter-names.txt

gobuster tftp -s tftp.example.test -w common-filenames.txt
```
## DIRB
...
Позволяет удобно перебирать все HTTP параметры.

Команда для `ffuf` - перебор всех комбинаций логин/пароль:

```bash
ffuf -w wordlists/usernames.txt:USER \
     -w ./wordlists/rockyou.txt:PASS \
     -X POST \
     -H "Content-Type: application/x-www-form-urlencoded" \
     -d "username=USER&password=PASS" \
     -u http://172.16.200.176:11498/login \
     -fc 401
```

### 📖 Объяснение команды

Давайте разберем, как `ffuf` выполняет задачу, аналогичную вашей исходной команде `hydra`.

- `-w wordlists/usernames.txt:USER`: Здесь мы указываем первый словарь и даём ему псевдоним `USER`. Теперь `ffuf` будет использовать этот псевдоним для подстановки значений из файла с именами пользователей.
- `-w ./wordlists/rockyou.txt:PASS`: Аналогично, указываем второй словарь и даём ему псевдоним `PASS`. Это ваш файл с паролями.
- `-X POST`: Задаёт HTTP-метод запроса — POST, так как мы отправляем данные формы.
- `-H 'key: value'` - задаёт HTTP заголовки. `"Content-Type: application/x-www-form-urlencoded"` особенно важен для брута логин форм.
- `-d "username=USER&password=PASS"`: Здесь передаются данные POST-запроса. `ffuf` автоматически подставит значения из соответствующих словарей вместо псевдонимов `USER` и `PASS`.
- `-u http://172.16.200.176:11498/login`: Целевой URL.
- `-fc 401`: Этот параметр позволяет **исключить** из результатов ответы с кодом `401 Unauthorized`. Это как раз то, о чём вы спрашивали ранее. `ffuf` умеет фильтровать ответы по HTTP-кодам, размерам и другим критериям.
- **Автоматическая настройка фильтров (`-ac`)**: Это очень мощная функция `ffuf`, которая может существенно упростить жизнь. Флаг `-ac` (auto-calibration) заставляет `ffuf` сначала сделать несколько «холостых» запросов, чтобы понять, как выглядят ответы по умолчанию (например, при неверном логине). После этого он автоматически отфильтрует эти «шумные» ответы, оставив в выдаче только аномалии, которые могут свидетельствовать об успешном входе.
