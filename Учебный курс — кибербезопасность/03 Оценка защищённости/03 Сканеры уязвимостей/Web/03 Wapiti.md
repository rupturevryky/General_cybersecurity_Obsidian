...
verified_against: https://github.com/wapiti-scanner/wapiti/wiki/Basic-usage
---

> [!note] Актуальные требования и scope
> Текущая версия Wapiti требует Python 3.12–3.14. Значение scope по умолчанию — `folder`; задавайте границу явно, чтобы сканер не вышел за разрешённый контур. Режим `punk` может переходить за пределы домена и для учебных работ запрещён.

```bash
python3 -m pip install --user wapiti3
wapiti --version
wapiti -u https://example.test/ --scope folder -v 1
```
...
wapiti -h
```
![[Учебный курс — кибербезопасность/03 Оценка защищённости/03 Сканеры уязвимостей/Web/assets/Pasted image 20240629182342.png]]
...
![[Учебный курс — кибербезопасность/03 Оценка защищённости/03 Сканеры уязвимостей/Web/assets/Pasted image 20240629182422.png]]
...
![[Учебный курс — кибербезопасность/03 Оценка защищённости/03 Сканеры уязвимостей/Web/assets/Pasted image 20240629182517.png]]
...
![[Учебный курс — кибербезопасность/03 Оценка защищённости/03 Сканеры уязвимостей/Web/assets/Pasted image 20240629182645.png]]
...
![[Учебный курс — кибербезопасность/03 Оценка защищённости/03 Сканеры уязвимостей/Web/assets/Pasted image 20240629182717.png]]
...
Wapiti просканирует весь сайт и выдаст соответствующий отчет. Чтобы исключить ненужные адреса (например, logout), добавьте параметр `-x <URL>`, а для авторизованного сканирования требуются куки. Формат и способ получения cookie-файла сверяйте через `wapiti --help` в установленной версии.
```
...
```
wapiti -u <URL> -x <EXCLUDED_URL> -c cookies.json
```
