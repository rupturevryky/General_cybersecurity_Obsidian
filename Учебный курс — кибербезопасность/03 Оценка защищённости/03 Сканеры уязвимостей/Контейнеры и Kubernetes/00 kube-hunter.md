...
verified_against: https://github.com/aquasecurity/kube-hunter
---

> [!danger] Разработка прекращена
> Aqua Security официально сообщает, что kube-hunter больше активно не развивается, и рекомендует Trivy для проверки Kubernetes misconfiguration и KBOM. Этот конспект сохранён как исторический материал; для новых лабораторных работ переходите к [[Учебный курс — кибербезопасность/03 Оценка защищённости/03 Сканеры уязвимостей/Контейнеры и Kubernetes/01 Trivy|Trivy]].

Если требуется воспроизвести старый сценарий, по умолчанию используйте пассивный режим и фиксируйте образ по digest:

```bash
docker pull aquasec/kube-hunter
docker run --rm aquasec/kube-hunter --list
docker run --rm aquasec/kube-hunter --remote <LAB_CLUSTER_ADDRESS>
```

Флаг `--active` способен менять состояние кластера и не должен использоваться в базовом курсе.
