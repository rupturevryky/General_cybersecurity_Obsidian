...
verified_against: https://docs.projectdiscovery.io/opensource/nuclei/install
---

# Nuclei

> [!success] Проверено
> Команды установки обновлены для Nuclei v3. Дистрибутивный пакет может отставать от официального релиза; всегда проверяйте `nuclei -version`.
...
Официальная установка через Go:
```bash
...
![[Учебный курс — кибербезопасность/03 Оценка защищённости/03 Сканеры уязвимостей/Автоматизация/assets/Nuclei шаблон.png]]
...
Если сканирование пришлось прекратить по какой-либо причине, и вы ввели комбинацию `ctrl+c`, Nuclei создаст файл возобновления и напечатает путь в выводе консоли:![[Учебный курс — кибербезопасность/03 Оценка защищённости/03 Сканеры уязвимостей/Автоматизация/assets/Nucle консоль.png]]
...
В Nuclei v3 для построчного JSON (JSONL), удобного для конвейеров и последующей обработки, используется `-j` (`-jsonl`):

```bash
nuclei -u https://example.com -jsonl
```

Для записи результата в файл используйте явные параметры экспорта:

```bash
nuclei -u https://example.com -json-export result.json
nuclei -u https://example.com -jsonl-export result.jsonl
```

Старый параметр `-json` не следует использовать в новых инструкциях: перед запуском сверяйте доступные ключи командой `nuclei -h`.
