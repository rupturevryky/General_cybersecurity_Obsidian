...
verified_against: https://www.openwall.com/john/doc/OPTIONS.shtml
---

# Инструменты проверки паролей и хешей

> [!note] Только офлайн-аудит разрешённых данных
> В John файл хешей является позиционным аргументом; результаты читаются через `john --show <HASH_FILE>`. Разделы John и Hashcat используйте только с тестовыми хешами, выданными внутри назначенного задания внешней платформы. Hydra и другие online-инструменты требуют отдельного разрешения и условий из [[Учебный курс — кибербезопасность/03 Оценка защищённости/04 Аудит учётных данных/04 Безопасная проверка аутентификации|безопасной проверки аутентификации]].
...
Пример:
```bash 
john --wordlist=/usr/share/wordlists/rockyou.txt --format=Raw-MD5 md5.hash
```
...
> [!danger] Online-проверка
> Hydra отправляет реальные попытки входа и способна заблокировать аккаунты или нарушить работу сервиса. Примеры ниже выполняются только против специально предоставленного учебного сервиса и test account с согласованными rate, attempt limit и stop condition.
...
JSON Web Token Toolkit версии 2 (https://github.com/ticarpi/jwt_tool) - это инструментарий для проверки, подделки, сканирования и фальсификации JWT (веб-токенов JSON).
...
```sh
flask-unsign --unsign --cookie "eyJ1c2VyIjoidGVzdCJ9.adfAQw.DztUL91GwVuswhl7VET5t1A-UyA" --wordlist /usr/share/wordlists/rockyou.txt
```

Когда ключ найден, можно подписывать cookie:

```sh
flask-unsign --sign --cookie "{'_user_id': 1, ...}" --secret 'НАЙДЕННЫЙ_СЕКРЕТ'
```
