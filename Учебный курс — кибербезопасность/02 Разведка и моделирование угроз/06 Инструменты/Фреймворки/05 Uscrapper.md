...
verified_against: https://github.com/z0m31en7/Uscrapper
---

# Uscrapper

> [!warning] Проверяйте установочный скрипт
> Официальный проект предлагает `install/install.sh`, но перед запуском стороннего shell-скрипта прочитайте его содержимое. Запускать основной файл нужно из корня репозитория, а URL необходимо передавать полностью.
...
./install/install.sh
python3 Uscrapper-vanta.py --help
python3 Uscrapper-vanta.py --url https://example.test --crawl 10 --generate-report
```
...
```
python3 Uscrapper-vanta.py --url https://example.test
```
