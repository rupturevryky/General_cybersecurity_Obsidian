...
verified_against: https://github.com/laramies/theHarvester/wiki/Quick-Start
---

# theHarvester

> [!success] Проверено
> Текущий список источников динамический; используйте `theHarvester -h`. Старый пример с backend `linkedin` больше не считается актуальным.
...
Безопасный пример с IANA-доменом и небольшим набором пассивных источников:
```bash
theHarvester -d example.com -b crtsh,certspotter
```

При запуске из checkout через `uv`:
```bash
...
`-b` — задаёт источники; актуальный перечень выводит `theHarvester -h`.

> [!warning] Устаревший список ниже
> Последующий перечень флагов сохранён как часть исходного конспекта и не является справкой для текущей версии. Источник истины — `theHarvester -h` и официальная wiki.
   Другие параметры:
...
![[Учебный курс — кибербезопасность/02 Разведка и моделирование угроз/06 Инструменты/Фреймворки/assets/Pasted image 20240706105738.png]]
