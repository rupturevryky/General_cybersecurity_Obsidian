---
topic: reconnaissance
level: intermediate
status: source-review-complete
last_reviewed: 2026-08-17
verified_against: https://owasp.org/www-project-web-security-testing-guide/latest/
---

> [!note] Проверка источников завершена
> Базовая линия: первичный профильный источник. Исторические payload-примеры ниже сохранены для разбора, но не считаются рекомендацией для реальных систем; практику выполняйте только в назначенной лаборатории внешней учебной платформы и в пределах её правил.

> [!note] Статус материала
> Исходный конспект сохранён, но команды, версии, зависимости и внешние ссылки необходимо сверять с официальной документацией перед практикой.

Инструкция по идентификации пользователей Google и Яндекс для слежки.

---

# OSINT по публичным идентификаторам

Идентификатор аккаунта, email, username и display name — разные сущности. Один человек может иметь несколько аккаунтов, а одно имя — принадлежать разным людям. Цель анализа должна быть связана с защитной задачей; поиск частной информации «на всякий случай» недопустим.

Правило атрибуции: один совпавший идентификатор создаёт гипотезу, несколько независимых признаков с согласованным временем могут повысить уверенность. Не используйте чувствительные признаки и не взаимодействуйте с человеком без отдельного разрешения.

## Google ID

Стоит учитывать, что у абсолютно любой почты может быть GoogleID, так как Google старается следить за всеми.

Узнать GoogleID и аватар пользователя по адресу электронной почты можно следующим способом:

1. Заходим в настройки (https://calendar.google.com/)
   
   ![[Учебный курс — кибербезопасность/02 Разведка и моделирование угроз/04 OSINT по идентификаторам/assets/Pasted image 20250418112534.png]]
   
2. Добавить календарь >> Подписаться на календарь
   
   ![[Учебный курс — кибербезопасность/02 Разведка и моделирование угроз/04 OSINT по идентификаторам/assets/Pasted image 20250418113322.png]]

3. Узнаём аватар пользователя (значение "`s`" позволяет менять размер):
   
   ![[Учебный курс — кибербезопасность/02 Разведка и моделирование угроз/04 OSINT по идентификаторам/assets/Pasted image 20250418114441.png]]

   ![[Учебный курс — кибербезопасность/02 Разведка и моделирование угроз/04 OSINT по идентификаторам/assets/Pasted image 20250418114227.png]]
   
\[2] Переходим в "Контакты". Щелкаем правой кнопкой мыши и выбираем "Просмотр кода страницы" (https://contacts.google.com/)
1. Через поиск находим нужный контакт

   ![[Учебный курс — кибербезопасность/02 Разведка и моделирование угроз/04 OSINT по идентификаторам/assets/Pasted image 20250418115515.png]]

2. Переходим в карточку пользователя

   ![[Учебный курс — кибербезопасность/02 Разведка и моделирование угроз/04 OSINT по идентификаторам/assets/Pasted image 20250418120533.png]]

3. Открываем исходный код (`crtl+U`)
4. Находим в нём саму почту, она будет на самом дне страницы:

   ![[Учебный курс — кибербезопасность/02 Разведка и моделирование угроз/04 OSINT по идентификаторам/assets/Pasted image 20250418122918.png]]
   
   5. Видим на 2м месте предыдущего массива и 9м месте следующего массива сам GoogleID:
   
   ![[Учебный курс — кибербезопасность/02 Разведка и моделирование угроз/04 OSINT по идентификаторам/assets/Pasted image 20250418123104.png]]

---

## Yandex ID

1. Перейдите в Яндекс Коллекции по ссылке `yandex.ru/collections/user/******`, где `******` - это никнейм пользователя (`никнейм@yandex.ru` (`никнейм@yandex.ru`))
2. Щелкаем правой кнопкой мыши и выбираем "Просмотр кода страницы"
3. Находим в коде пункт "cover_info". Перед ней будет пользовательский :ID (:12345678)

---

## Получение данных

> Google:
- `google.com/maps/contrib/*GoogleID*` - Google карты, отзывы
- `get.google.com/albumarchive/*GoogleID*` - публичный Google фотоальбом 
- `drive.google.com/drive/search?q=owner:*GoogleMail*` - публичный Google Drive
- `plus.google.com/*GoogleID*` - 

> Yandex:
- `yandex.ru/collections/user/*nickname*/` - Yandex Коллекции
- `n.maps.yandex.ru/#/users/*YandexID*` - Yandex Карты
- `music.yandex.ru/user/*nickname*/` - Yandex Музыка
