...
verified_against: https://www.rfc-editor.org/rfc/rfc9700
---

> [!note] Проверка источников завершена
> Базовая линия: RFC 9700 / BCP 240. Исторические payload-примеры ниже сохранены для разбора, но не считаются рекомендацией для реальных систем; практику выполняйте только в назначенной лаборатории внешней учебной платформы и в пределах её правил.
...
Существует несколько способов реализации OAuth — потоки (flows), основанные на типах грантов. Для пользовательских клиентов современной базовой линией является **Authorization Code с PKCE**. Implicit Flow сохранён ниже только для распознавания и тестирования наследуемых реализаций: RFC 9700 не рекомендует выдавать access token в authorization response.
...
Ниже подробно разобраны **Authorization Code** и исторический **Implicit Flow**. Для новых приложений выбирайте Authorization Code с PKCE; Implicit нужен специалисту прежде всего для аудита старых систем.
...
![[Учебный курс — кибербезопасность/04 Безопасность приложений/03 Аутентификация и сессии/assets/Pasted image 20260131194819.png]]
...
4. `scope`
   ...
   > Используется для указания, к какому подмножеству данных пользователя клиентское приложение хочет получить доступ. Обратите внимание, что это могут быть пользовательские области, заданные провайдером OAuth, или стандартизированные области, определённые спецификацией [[Учебный курс — кибербезопасность/04 Безопасность приложений/03 Аутентификация и сессии/04 OAuth 2.0 и OpenID Connect#OpenID Connect|OpenID Connect]].
   > 
5. `state`
...
![[Учебный курс — кибербезопасность/04 Безопасность приложений/03 Аутентификация и сессии/assets/Pasted image 20260131202051.png]]
...
Implicit Flow исторически применялся в одностраничных приложениях, но больше не рекомендуется: токен оказывается в authorization response через браузер и хуже защищён от утечки и подмены. Современная замена — Authorization Code с PKCE.
...
Лаба: [[Учебный курс — кибербезопасность/04 Безопасность приложений/03 Аутентификация и сессии/04 OAuth 2.0 и OpenID Connect#Lab: Authentication bypass via OAuth implicit flow|Authentication bypass via OAuth implicit flow]].
...
Лаба: [[Учебный курс — кибербезопасность/04 Безопасность приложений/03 Аутентификация и сессии/04 OAuth 2.0 и OpenID Connect#Lab: Forced OAuth profile linking|Forced OAuth profile linking]]
...
3. В запросе `GET /auth?client_id=...` в параметре `redirect_uri` нельзя изменить основную страницу, но можно дописывать путь, например с помощью `/../../` или `../post?postId=1`, при чём токен доступа будет включен в запрос как hash-параметр:
   ...
   ![[Учебный курс — кибербезопасность/04 Безопасность приложений/03 Аутентификация и сессии/assets/Pasted image 20260202171948.png]]
...
3. Пробуем:
   ...
   ![[Учебный курс — кибербезопасность/04 Безопасность приложений/03 Аутентификация и сессии/assets/Pasted image 20260206140335.png]]
   
   ![[Учебный курс — кибербезопасность/04 Безопасность приложений/03 Аутентификация и сессии/assets/Pasted image 20260206140359.png]]
   
   ![[Учебный курс — кибербезопасность/04 Безопасность приложений/03 Аутентификация и сессии/assets/Pasted image 20260206140536.png]]
   
   ![[Учебный курс — кибербезопасность/04 Безопасность приложений/03 Аутентификация и сессии/assets/Pasted image 20260206140623.png]]
...
4. На странице подтверждения разрешений OAuth мы видим логотип клиентского приложения:
   ...
   ![[Учебный курс — кибербезопасность/04 Безопасность приложений/03 Аутентификация и сессии/assets/Pasted image 20260206141233.png]]
   
   Он загружается из `/client/CLIENT-ID/logo`. 
   
   ![[Учебный курс — кибербезопасность/04 Безопасность приложений/03 Аутентификация и сессии/assets/Pasted image 20260206141329.png]]
...
5. Добавим `logo_uri` в свой зарегистрированный сайт со ссылкой на collaborator:
   ...
   ![[Учебный курс — кибербезопасность/04 Безопасность приложений/03 Аутентификация и сессии/assets/Pasted image 20260206141831.png]]
   
   Проверим, работает ли запрос в колаборатор:
   
   ![[Учебный курс — кибербезопасность/04 Безопасность приложений/03 Аутентификация и сессии/assets/Pasted image 20260206142001.png]]
   
   ![[Учебный курс — кибербезопасность/04 Безопасность приложений/03 Аутентификация и сессии/assets/Pasted image 20260206142024.png]]
...
1. Регистрируем на месте иконки endpoint из лабораторной:
   ...
   > Получаем данные:
   > 
   > ![[Учебный курс — кибербезопасность/04 Безопасность приложений/03 Аутентификация и сессии/assets/Pasted image 20260206142606.png]]
