...
verified_against: https://docs.metasploit.com/docs/using-metasploit/intermediate/metasploit-database-support.html
---

> [!note] Актуальная инициализация
> В Kali база запускается через `sudo msfdb init`, состояние проверяется `sudo msfdb status`. Повторная инициализация может затронуть существующую базу.
...
![[Учебный курс — кибербезопасность/05 Сетевая безопасность/Техники сетевого тестирования/assets/Pasted image 20240630093625.png]]Используйте следующие команды, чтобы установить фактическое целевое веб-приложение. Для примера, буду использовать веб-приложение [Mutillidae](https://spy-soft.net/install-mutillidae-docker/) виртуальной машины **OWASP BWA**:
```
msf6 > wmap_targets -t http://172.30.1.23/mutillidae/
```
![[Учебный курс — кибербезопасность/05 Сетевая безопасность/Техники сетевого тестирования/assets/Pasted image 20240630093742.png]]
...
![[Учебный курс — кибербезопасность/05 Сетевая безопасность/Техники сетевого тестирования/assets/Pasted image 20240630093822.png]]
...
![[Учебный курс — кибербезопасность/05 Сетевая безопасность/Техники сетевого тестирования/assets/Pasted image 20240630093920.png]]
Итак, вы узнали, как выполнять поиск уязвимостей веб-приложения с помощью **Metasploit** на **Kali Linux**. Связанный материал: [[Учебный курс — кибербезопасность/03 Оценка защищённости/03 Сканеры уязвимостей/Web/01 Nikto|Nikto]].
