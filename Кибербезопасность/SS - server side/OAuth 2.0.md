...
При использовании OAuth для аутентификации, OAuth часто расширяется с помощью уровня OpenID Connect, который предоставляет некоторые дополнительные функции, связанные с идентификацией и аутентификацией пользователей. 
...
Изначально OAuth не был разработан с учетом аутентификации; он был предназначен для делегирования авторизации для определенных ресурсов между приложениями. Но простые механизмы аутентификации OAuth были далеки от идеала. Во-первых, клиентское приложение не имело возможности узнать, когда, где и как был аутентифицирован пользователь. Поскольку каждая из этих реализаций была своего рода индивидуальным обходным решением, не существовало также стандартного способа запроса пользовательских данных для этой цели. Для правильной поддержки OAuth клиентские приложения должны были настраивать отдельные механизмы OAuth для каждого провайдера, каждый с разными конечными точками, уникальными наборами областей действия и т. д.

OpenID Connect решает многие из этих проблем, добавляя стандартизированные функции, связанные с идентификацией, чтобы аутентификация через OAuth работала более надежно и единообразно.
...
Другим важным нововведением OpenID Connect является тип ответа `id_token`. Он возвращает веб-токен JSON (JWT), подписанный веб-подписью JSON (JWS). Полезная нагрузка JWT содержит список заявлений (claims), основанных на первоначально запрошенной области действия. Она также содержит информацию о том, как и когда пользователь был в последний раз аутентифицирован службой OAuth. Клиентское приложение может использовать эту информацию, чтобы решить, была ли аутентификация пользователя достаточной.
