...
Хотя вы можете не иметь возможности запускать скрипты на сервере, вы всё равно можете загружать скрипты для атак на стороне клиента. Например, если вы можете загружать HTML-файлы или SVG-изображения, вы потенциально можете использовать тег `<script>` для создания хранимых XSS-полезных нагрузок. 

---

## Использование уязвимостей при разборе загруженных файлов

Если загруженный файл, похоже, хранится и обслуживается безопасно, последний вариант — попытаться использовать уязвимости, специфичные для разбора или обработки различных форматов. Например, вы знаете, что сервер парсирует файлы на основе XML, такие как Microsoft Office `.doc` или файлы `.xls`, это может быть потенциальным источником для атак с помощью XXE-инъекций. 

---

## Загрузка файлов с помощью PUT

Стоит отметить, что некоторые веб-серверы могут быть настроены на поддержку запросов `PUT`. Если не предусмотрены соответствующие меры защиты, это может предоставить альтернативный способ загрузки вредоносных файлов, даже если функция загрузки недоступна через веб-интерфейс. 
...
Вы можете попробовать отправлять `OPTIONS` запросы разным конечным точкам, чтобы проверить, нет ли тех, кто поддерживает `PUT` метод. 

---

## Как предотвратить уязвимости при загрузке файлов

Разрешить пользователям загружать файлы — обычное дело, и это не обязательно опасно, если принимать правильные меры предосторожности. В целом, самый эффективный способ защитить свои сайты от этих уязвимостей — применять все следующие практики:

- Проверьте расширение файла по белому списку, а не по чёрному. 
- Убедитесь, что имя файла не содержит подстрок, которые можно интерпретировать как каталог или последовательность обхождения (`../`). 
- Переименуйте загруженные файлы, чтобы избежать столкновений, которые могут привести к перезаписи существующих файлов.
- Не загружайте файлы в постоянную файловую систему сервера, пока они не будут полностью проверены.
- По возможности используйте устоявшийся фреймворк для предварительной обработки загрузки файлов, а не пытайтесь создавать собственные механизмы валидации.